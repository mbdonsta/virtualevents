...
@section('content')
    @include('backend.components.page_title', ['title' => __('Create Event')])
                                ...
                                <div class="col-md-6">
                                    <div class="mb-10">
                                        ...
                                        <textarea class="form-control @error('title') is-invalid @enderror" rows="2"
                                                  name="title">{{ nl2br(old('title')) }}</textarea>
                                        @error('title')
                                    ...
                                    <div class="mb-10">
                                        ...
                                        <select class="form-select @error('title_option') is-invalid @enderror"
                                            ...
                                            @foreach ($titleOptions as $option)
                                                <option
                                                    value="{{ $option['id'] }}" {{ $option['id'] == old('title_option') ? 'selected' : '' }}>
                                                    {{ $option['name'] }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-10">
                                        ...
                                        <select class="form-select @error('language_id') is-invalid @enderror"
                                            ...
                                            @foreach ($languages as $language)
                                                <option
                                                    value="{{ $language->id }}" {{ $language->id == old('language_id') ? 'selected' : '' }}>{{ $language->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('language_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-10">
                                        ...
                                        <select class="form-select @error('is_public') is-invalid @enderror"
                                                name="is_public">
                                            <option
                                                value="0" {{ 0 == old('is_public') ? 'selected' : '' }}>{{ __('Private') }}</option>
                                            <option
                                                value="1" {{ 1 == old('is_public') ? 'selected' : '' }}>{{ __('Public') }}</option>
                                        </select>
                                        @error('is_public')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
