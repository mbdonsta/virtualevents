...
@section('event_wrapper_content')
    ...
    <div class="card card-flush">
        ...
        <div class="card-body">
            <form
                action="{{ route('backend.stand_items.update', ['exhibitionStandItem' => $exhibitionStandItem->id]) }}"
                method="POST">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-10">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                   name="name" value="{{ old('name', $exhibitionStandItem->name) }}">
                            @error('name')
                        ...
                        <div class="mb-10">
                            ...
                            <select class="form-select @error('item_type') is-invalid @enderror"
                                ...
                                @foreach ($itemTypes as $index => $itemType)
                                    <option
                                        value="{{ $index }}" {{ $index == old('item_type', $exhibitionStandItem->item_type) ? 'selected' : '' }}>
                                        {{ $itemType }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div
                            ...
                            {!! !in_array(old('item_type', $exhibitionStandItem->item_type), [
                            ...
                            <input type="text" class="form-control @error('url') is-invalid @enderror"
                                   name="url" value="{{ old('url', $exhibitionStandItem->url) }}">
                            @error('url')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div
                            ...
                            {!! old('item_type', $exhibitionStandItem->item_type) != \App\Models\ExhibitionStandItem::ITEM_TYPE_DOWNLOAD_FILE ? 'style="display: none;"' : '' !!}
                        >
                            <label class="form-label">
                                {{ __('Download file') }}
                                <small class="text-gray-400">({{ __('required') }})</small>
                            ...
                            <image-upload
                                ...
                                file_id="{{ old('download_file_id', $exhibitionStandItem->download_file_id) }}"
                                file_url="{{ old('download_file_ul', $exhibitionStandItem->getDownloadFileUrl()) }}"
                                field_name="download_file_id"
                                field_url_name="download_file_url"
                                filename="{{ old('download_filename', $exhibitionStandItem->getDownloadFilename()) }}"
                                upload_type="file"
                            ></image-upload>
                    ...
                    <div class="col-md-6">
                        <div
                            ...
                            {!! old('item_type', $exhibitionStandItem->item_type) == \App\Models\ExhibitionStandItem::ITEM_TYPE_BANNER_FROM_EXTERNAL_URL ? 'style="display: none;"' : '' !!}>
                            <label class="form-label">
                                {{ __('Item image') }}
                                <small class="text-gray-400">({{ __('required') }})</small>
                            </label>
                            <image-upload
                                ...
                                file_id="{{ old('banner_file_id', $exhibitionStandItem->banner_file_id) }}"
                                file_url="{{ old('banner_file_url', $exhibitionStandItem->getBannerImageUrl()) }}"
