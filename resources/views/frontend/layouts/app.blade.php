...
<!--begin::Head-->
<head>
    <title>{{ isset($event) ? $event->title : 'Streamie' }}</title>
    <meta charset="utf-8"/>
...
<body id="kt_app_body" class="frontend-body" data-kt-app-layout="light-header"
...
<div class="d-flex flex-column flex-root app-root" id="app">
    <div class="app-page flex-column flex-column-fluid" id="kt_app_page">
        ...
        <div class="app-wrapper flex-column flex-row-fluid" id="kt_app_wrapper">
            <!--begin::Main-->
            <div class="app-main flex-row-fluid" id="kt_app_main">
                <!--begin::Content wrapper-->
                @yield('content')
                <!--end::Content wrapper-->
                <!--begin::Footer-->
                <div id="kt_app_footer" class="app-footer h-auto">
                    ...
                    <div
                        ...
                        <ul class="event-navigation">
                                ...
                                <li class="admin-zone">
                                    <a href="{{ route('backend.events.edit', ['event' => $event->id]) }}"
                                       class="menu-link">
                                        ...
                                        <span class="menu-title">{{ __('Manage') }}</span>
                                    </a>
                                </li>
                            @endcan
                            @if ($event->hasProgramDays())
                                <li>
                                    <a href="{{ route('frontend.events.program.view', ['slug' => $event->slug]) }}"
                                       class="menu-link {{ Route::currentRouteName() === 'frontend.events.program.view'? 'active' : '' }}">
                                        ...
                                        <span class="menu-title">{{ __('Program') }}</span>
                                    </a>
                                </li>
                            @endif
                            @if ($event->hasRooms())
                                <li class="stream-item">
                                    @if (Route::input('eventRoom'))
                                        @foreach($event->getRooms() as $eventRoom)
                                            @if (Route::input('eventRoom')->id === $eventRoom->id)
                                                <a href="{{ route('frontend.events.room.view', ['slug' => $event->slug, 'eventRoom' => $eventRoom->id]) }}"
                                                   class="menu-link {{ Route::input('eventRoom') && Route::currentRouteName() === 'frontend.event.room.view' && Route::input('eventRoom')->id === $eventRoom->id ? 'active' : '' }}">
                                                    <svg fill="#000000" viewBox="0 0 32 30"
                                        ...
                                        @endforeach
                                    @else
                                        <a href="{{ route('frontend.events.room.view', ['slug' => $event->slug, 'eventRoom' => $event->getRooms()[0]->id]) }}"
                                           class="menu-link">
                                            <svg fill="#000000" viewBox="0 0 32 30"
                                                 version="1.1" xmlns="http://www.w3.org/2000/svg">
                                                <title>video-camera</title>
                                            ...
                                            <span class="menu-title">{{ $event->getRooms()[0]->name }}</span>
                                        </a>
                                    @endif
                                    @if (count($event->getRooms()) > 1)
                                                    ...
                                                    @if (Route::input('eventRoom')->id !== $eventRoom->id)
                                                        <a href="{{ route('frontend.events.room.view', ['slug' => $event->slug, 'eventRoom' => $eventRoom->id]) }}"
                                                           class="menu-link">
                                                            ...
                                                            <span
                                                                class="menu-title">{{ $eventRoom->name }}</span>
                                                        </a>
                                                    @endif
                                                @else
                                                    @if ($event->getRooms()[0]->id !== $eventRoom->id)
                                                        <a href="{{ route('frontend.events.room.view', ['slug' => $event->slug, 'eventRoom' => $eventRoom->id]) }}"
                                                           class="menu-link">
                                                            <svg fill="#000000" viewBox="0 0 32 30"
                                                                 version="1.1" xmlns="http://www.w3.org/2000/svg">
                            ...
                            @endif
                            @if (!$event->exhibitionGroups->isEmpty())
                                <li>
                                    <a href="{{ route('frontend.events.exhibition.view', ['slug' => $event->slug]) }}"
                                       class="menu-link {{ Route::currentRouteName() === 'frontend.events.exhibition.view'? 'active' : '' }}">
                                        <svg viewBox="0 0 21 21" xmlns="http://www.w3.org/2000/svg">
                                            <g fill="none" fill-rule="evenodd" stroke="#000000" stroke-linecap="round"
                                               stroke-linejoin="round" transform="translate(1 2)">
                                                <path
                                                    d="m16.5 12.5v-10.01471863h-14v10.01471863c0 .5522847.44771525 1 1 1h12c.5522847 0 1-.4477153 1-1z"/>
                                                <path d="m7.5 13.5-2 3.5"/>
                                                <path d="m13.5 13.5-2 3" transform="matrix(-1 0 0 1 25 0)"/>
                                                <path d="m.5 2.5h18"/>
                                                <path
                                                    d="m9.49894742.49789429c1.05502148.00261296 1.91822238.81840641 1.99543358 1.85289779l.0056181.1492082-4.00000003-.00210599c-.00105165-1.1045695.89437885-2 1.99894835-2z"/>
                                            </g>
                                        </svg>
                                        <span class="menu-title">{{ __('Exhibition') }}</span>
                                    </a>
                                </li>
                            @endif
                            @if (!$event->eventPosters->isEmpty())
                                <li>
                                    <a href="{{ route('frontend.events.posters.index', ['slug' => $event->slug]) }}"
                                       class="menu-link {{ Route::currentRouteName() === 'frontend.events.posters.index'? 'active' : '' }}">
                                        ...
                                        <span class="menu-title">{{ __('Posters') }}</span>
                                    </a>
                                </li>
                            @endif
                            <li class="logout">
